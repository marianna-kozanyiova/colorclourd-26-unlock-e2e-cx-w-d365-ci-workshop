...
This journey is designed to support customers after purchase in a more personalized way. Customers whose product has been shipped will first receive an onboarding email. The journey will test two subject line variants through an A/B test. It will then branch based on whether the customer clicked the **Download the app** link. Customers who clicked the link will continue into channel optimization, while customers who did not click the link will receive a text message reminder. The journey will later support an accessory upsell path using preconfigured content.
